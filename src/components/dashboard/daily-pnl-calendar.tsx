...
export function DailyPnlCalendar() {
  const {
    summary,
    dailyPnlResults: providerResults,
  ...
  const isCoveredByProvider = analysisYear
    ? currentMonth.getFullYear() === analysisYear
    : isSameMonth(currentMonth, new Date());

  const { dailyPnlResults: hookResults, loading: hookLoading, status: hookStatus } =
    useDailyPnl(currentMonth);

  // [DEBUG] Snapshot-first logging
  React.useEffect(() => {
    if (hookStatus === 'FRESH' && !hookLoading) {
      console.log(`[Snapshot] ${format(currentMonth, 'yyyy-MM')} is FRESH (Snapshot Hit)`);
    } else if (hookStatus === 'STALE_RECOMPUTING') {
      console.warn(`[Snapshot] ${format(currentMonth, 'yyyy-MM')} is STALE. Recomputing...`);
    }
  }, [hookStatus, hookLoading, currentMonth]);
  ...
  const renderStatus = isCoveredByProvider ? 'FRESH' : hookStatus;

  // Calculate total PnL for the month
  const monthPnl = React.useMemo(() => {
    let total = 0;
    Object.values(dailyPnlResults).forEach((res: any) => {
      if (res.date?.startsWith(format(currentMonth, "yyyy-MM"))) {
        // For active provider data, status should be respected.
        // Note: In Time Travel (2025), dates are 'past', so status might be 'market_closed' or 'ok'.
        if (
          res.status === "ok" ||
          res.status === "partial" ||
          res.status === "market_closed"
        ) {
          total += res.totalPnl;
        }
      }
  ...
  const missingItems = React.useMemo(() => {
    ...
    import('@/lib/debug/eod-timeline').then(({ audit }) => {
      audit("UI.missingItems.compute", {
        missingItemsCount: items.length,
        sample: items.slice(0, 3).map(it => `${it.date}: ${it.symbols.join(',')}`)
      });
    });
      ...
      <div className="relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950/50 backdrop-blur-xl shadow-2xl">
        ...
        {isCalculating && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-zinc-950/60 backdrop-blur-sm transition-all duration-300">
            ...
            <span className="text-xs font-medium text-emerald-400/80 tracking-wider">
              Updating Data...
            </span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between border-b border-white/5 bg-white/5 px-4 py-3 backdrop-blur-md">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-medium tracking-wide text-zinc-100">
              每日盈亏日历 (Total PnL)
            </h3>
            ...
            {isCalculating || loading ? (
              <div className="flex items-center gap-1.5 ml-3 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                ...
                <span className="text-[10px] font-medium text-emerald-400">
                  {renderStatus === 'STALE_RECOMPUTING' ? 'RECALC...' : 'LOADING...'}
                </span>
              </div>
            ) : (
              <div className={cn(
                "flex items-center gap-1.5 ml-3 px-2 py-0.5 rounded-full border text-[10px] font-bold font-mono tracking-tighter",
                renderStatus === 'FRESH' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                  renderStatus === 'SAFE' ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : // [NEW] Circuit Breaker Active
                    renderStatus === 'STALE_RECOMPUTING' ? "bg-amber-500/10 border-amber-500/20 text-amber-500" :
                      "bg-zinc-800 border-zinc-700 text-zinc-500"
              )}>
                {renderStatus === 'FRESH' ? 'FRSH' :
                  renderStatus === 'SAFE' ? 'SAFE' : // [NEW] Safe-Mode Text
                    renderStatus === 'STALE_RECOMPUTING' ? 'STAL' :
                      renderStatus === 'MISSING_RECOMPUTING' ? 'MISS' : 'EVL'}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className="px-2 py-1 rounded-md hover:bg-white/10 transition-colors">
                  <span className="w-20 text-center font-mono text-sm font-bold text-zinc-200">
                    {format(currentMonth, "yyyy-MM")}
                  </span>
                </button>
              </PopoverTrigger>
              <PopoverContent className="w-64 border-zinc-800 bg-zinc-950 p-3 shadow-2xl" align="center">
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between border-b border-white/10 pb-2">
                    <button
                      onClick={() => setCurrentMonth(subMonths(currentMonth, 12))}
                      className="rounded-md p-1 hover:bg-white/10 text-zinc-400 hover:text-white transition-all"
                    >
        ...
        {openAudit && (
          <AuditDialog open={openAudit} onClose={() => setOpenAudit(false)} />
        )}
        ...
        <div
          className={cn(
            ...
            isCalculating && "opacity-60 grayscale-[0.3]",
          )}
        >
          ...
          <div className="grid grid-cols-7 gap-1.5 md:gap-2">
            {days.map((day, dayIdx) => {
              ...
              let isMissingData = res?.status === "missing_data";
              const isLoadingEod = res?.status === "loading_eod";
              let isMarketClosed = res?.status === "market_closed";
              const isFetchIncomplete = res?.status === "fetch_incomplete";
              const isIntraday = res?.status === "intraday";
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isToday
                      ? "border-emerald-500/50 ring-1 ring-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)] z-10"
                      : "border-white/5",
                    ...
                    !isMissingData &&
                    isProfit &&
                    "bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20 lg:hover:border-emerald-500/40 lg:hover:from-emerald-500/20",
                    !isMarketClosed &&
                    hasData &&
                    !isMissingData &&
                    isLoss &&
                    "bg-gradient-to-br from-rose-500/10 to-rose-500/5 border-rose-500/20 lg:hover:border-rose-500/40 lg:hover:from-rose-500/20",
                    ...
                    "bg-white/5 backdrop-blur-[2px] border-white/5 lg:hover:bg-white/10 transition-colors",

                    // Holiday
                    isMarketClosed &&
                    res?.marketClosedReason !== "Weekend" &&
                    "bg-indigo-900/30 shadow-[inset_0_0_10px_rgba(49,46,129,0.4)] border-indigo-500/30",

                    // Hover Lift
                    !isToday &&
                    "lg:hover:-translate-y-1 lg:hover:shadow-lg lg:hover:z-10",
                  ...
                  title={
                    isMissingData
                      ? "EOD数据待更新"
                      : isIntraday
                        ? "盘中交易中 (Intraday) / 结算价将在延后 16:00 后生成"
                        : isFetchIncomplete
                          ? "加载未完成 (fetch_incomplete) / 拉取受限，请刷新或稍后重试"
                          : isMarketClosed
                            ? `市场休市: ${res?.marketClosedReason || "非交易日"}`
                            : hasData
                              ? `Total: ${fmtFull(total)}${isPartial ? "\n(包含估算价格: Plan Limit/No Liquidity 已沿用成本或前收盘)" : ""}\n存量(Stock): ${fmtFull(unrealizedChange)}\n增量(Incr): ${fmtFull(realized)}`
                              : undefined
                  }
                >
                  {/* Date Number */}
                  ...
                  {isMarketClosed ? (
                    <div className="flex flex-col items-center justify-center flex-1 w-full px-1">
                      ...
                      {res?.marketClosedReason === "Weekend" ? (
                        ...
                        <div className="flex flex-col items-center gap-1 opacity-70">
                          ...
                          <span className="text-[10px] font-serif italic tracking-widest text-amber-100 drop-shadow-md">
                            Holiday
                          </span>
                        </div>
                      ) : (
                  ...
                  ) : hasData && !isMissingData && !isIntraday ? (
                    <div className="flex flex-col gap-1 my-1">
                      <div
                        className={cn(
                          ...
                          Math.abs(total!) > 99999
                            ? "text-lg md:text-xl"
                            : "text-xl md:text-2xl", // Adaptive size
                          isProfit
                      ...
                      {res?.earlyCloseReason && (
                        <div className="text-[8px] font-bold text-amber-500/80 uppercase tracking-tighter text-center mt-1 scale-90">
                          {res.earlyCloseReason.replace('Early Close (', '').replace(')', '')}
                        </div>
                      )}
                    </div>
                  ) : isIntraday ? (
                    <div className="flex items-center justify-center flex-1 bg-emerald-500/10 w-full rounded border border-emerald-500/20">
                      <span className="text-[10px] font-bold text-emerald-400">
                        盘中
                      </span>
                    </div>
                  ) : isFetchIncomplete ? (
                    <div className="flex items-center justify-center flex-1 bg-zinc-800/20 w-full rounded border border-zinc-700/30">
                      <span className="text-[10px] font-bold text-zinc-500">
                        加载未完成
                      </span>
                    </div>
                  ) : isLoadingEod ? (
                    <div className="flex items-center justify-center flex-1 bg-emerald-500/5 w-full rounded border border-emerald-500/10">
                      <Loader2 className="h-3 w-3 animate-spin text-emerald-400/50" />
                      <span className="text-[9px] ml-1 font-bold text-emerald-500/60 uppercase tracking-tighter">
                        同步中...
                      </span>
                    </div>
                  ) : isMissingData ? (
                    ...
                    <div className="flex items-center justify-center flex-1">
                      <span className="text-[10px] font-medium text-zinc-800/50">
                        未开盘
                      </span>
                    </div>
                  ) : null}
                    ...
                    <div className="grid grid-cols-3 gap-0.5 border-t border-white/10 pt-1.5 mt-1">
                      ...
                      <div className="flex flex-col items-start min-w-0">
                        <HoverCard openDelay={200} closeDelay={100}>
                          ...
                          >
                            <span className="text-[9px] md:text-[10px] font-bold text-zinc-500 uppercase tracking-wider leading-none mb-0.5 cursor-help">
                              持仓
                            </span>
                          </HoverCardTrigger>
                            ...
                            <div className="space-y-2">
                              ...
                              <ul className="list-disc list-inside space-y-1 text-zinc-400">
                                <li>
                                  ...
                                  ：所有未平仓位（无论新旧）在今日的市值波动。
                                </li>
                                <li>
                                  <span className="text-zinc-200">
                                    隔夜结转
                                  </span>
                                  ：卖出昨日以前持仓所产生的已实现盈亏 (Realized
                                  M4)。
                                </li>
                              </ul>
                            </div>
                      ...
                      <div className="flex flex-col items-center min-w-0 border-l border-r border-white/5">
                        <HoverCard openDelay={200} closeDelay={100}>
                          <HoverCardTrigger
                            ...
                            onClick={(e) => e.stopPropagation()}
                          >
                            <span className="text-[9px] md:text-[10px] font-bold text-indigo-400/70 uppercase tracking-wider leading-none mb-0.5 cursor-help">
                              账本
                            </span>
                          </HoverCardTrigger>
                          <HoverCardContent className="w-64 bg-zinc-950 border-zinc-800 text-zinc-300 text-xs p-3 shadow-xl backdrop-blur-xl">
                      ...
                      <div className="flex flex-col items-end min-w-0">
                        <HoverCard openDelay={200} closeDelay={100}>
                          <HoverCardTrigger
                            ...
                            onClick={(e) => e.stopPropagation()}
                          >
                            <span className="text-[9px] md:text-[10px] font-bold text-amber-500/70 uppercase tracking-wider leading-none mb-0.5 cursor-help">
                              撮合
                            </span>
                          </HoverCardTrigger>
                          <HoverCardContent className="w-64 bg-zinc-950 border-zinc-800 text-zinc-300 text-xs p-3 shadow-xl backdrop-blur-xl">
            ...
            <div className="border-t border-emerald-500/20 bg-emerald-500/5 backdrop-blur-md p-3 animate-in slide-in-from-top-2 duration-300">
              ...
              <div className="flex flex-col gap-1 ml-5">
                <div className="flex justify-between text-[10px] font-mono text-emerald-400/80">
                  <span>
                    Task: {autoHealProgress.current} / {autoHealProgress.total}
                  </span>
                  <span>
                    {Math.round(
                      (autoHealProgress.current / autoHealProgress.total) * 100,
                    )}
                    %
                  </span>
                </div>
                ...
                <div className="h-1 w-full bg-emerald-500/20 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(autoHealProgress.current / autoHealProgress.total) * 100}%`,
                    }}
                  />
                </div>
              ...
              <div className="border-t border-rose-500/20 bg-rose-500/5 backdrop-blur-md p-3 animate-in slide-in-from-top-2 duration-300">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-[10px] font-bold uppercase tracking-wider text-rose-500">
                      缺失数据清单 (Missing EOD)
                    </h4>
                  </div>
                  <span className="text-[9px] text-zinc-500 bg-zinc-900/50 px-1.5 py-0.5 rounded border border-white/5">
                    请点击上方「审计诊断」按钮进行同步
                  </span>
                </div>
