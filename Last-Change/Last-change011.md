*   **纽约**: 2024-07-31 18:00:00 (America/New_York)
*   **UTC**: 2024-07-31 22:00:00 (UTC)
...
*   **修改前**:
    ...
    function parseLegacyCnUtcOffset(s: string): number | null {
      // 形如：2025年10月20日 UTC-4 09:56:19 或 2025年7月3日 UTC+8 9:05
      ...
      const day = Number(d);
      const offsetHours = Number(off); // 例：-4 / +8
      ...
      // 本地(带UTC偏移) → UTC：UTC = local - offset
      // 例如 "UTC-4 09:56:19" => UTC = 09:56:19 + 4h
...
*   **修改后 (新文件内容)**:
    ...
    function normalizeFireTx(raw: FireTx, source: 'transactions' | 'trades'): Tx | null {
      ...
      const rawType = (raw.type || raw.action || (raw.side as any)) as TxType | string | undefined;
      const tLower = (rawType ?? (qtyNum !== null && qtyNum >= 0 ? 'Buy' : 'Sell')).toString().toLowerCase();
      let side: Side;
      let qty = qtyNum;
