*   **纽约**: 2024-08-01 12:00:00 (America/New_York)
*   **UTC**: 2024-08-01 16:00:00 (UTC)

---

#### **1. 修改文件： `src/components/dashboard/transaction-history.tsx`**
...
*   **修改前**:
    ...
    function AssetBadge({ assetType }: { assetType: Tx['assetType'] }) {
      const text = assetType === 'option' ? '期权' : '股票';
      return (
    ...
    function ActionBadge({ opKind, isOption }: { opKind: OpKind; isOption: boolean }) {
      // 股票色板
      const STOCK = {
        BUY:  'bg-emerald-600',
        SELL: 'bg-red-600',
      ...
      const cls = isOption
        ? (opKind==='BTO'?OPT.BTO:opKind==='STO'?OPT.STO:opKind==='STC'?OPT.STC:OPT.BTC)
        : (opKind==='BUY'?STOCK.BUY:opKind==='SELL'?STOCK.SELL:opKind==='SHORT'?STOCK.SHORT:STOCK.COVER);

      const Icon = ((): any => {
        if (opKind==='SHORT'||opKind==='STO') return TrendingDown;
        if (opKind==='COVER'||opKind==='BTC') return Undo2;
        if (opKind==='SELL'||opKind==='STC')  return ArrowDownLeft;
        return ArrowUpRight; // BUY/BTO
      })();

      return (
        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-white ${cls}`}>
          <Icon className="w-3.5 h-3.5" />
          <span>{
            opKind==='BTO'?'买入':opKind==='STO'?'卖空':opKind==='STC'?'卖出':
            opKind==='BTC'?'补回':opKind==='SELL'?'卖出':opKind==='SHORT'?'卖空':
            opKind==='COVER'?'补回':'买入'
          }</span>
          {isOption && <Sparkles className="w-3 h-3 opacity-90" />} {/* 期权标记 */}
        </span>
      );
    }
                      ...
                      {!loading && !error &&
                        rows.map((tx) => (
                          <TableRow key={`${'\'\'\''}tx.source`-'\'\''}${tx.id}`}>
                            ...
                            <TableCell><ActionBadge opKind={tx.opKind} isOption={tx.isOption} /></TableCell>
...
*   **修改后 (新文件内容)**:
    ...
    // 让“类型”列用不同底色区分期权 vs 股票（只影响类型列）
    function AssetBadge({ assetType }: { assetType: Tx['assetType'] }) {
      const isOption = assetType === 'option';
      return (
        <Badge className={cn('border-none gap-1', isOption ? 'bg-cyan-600 text-white' : 'bg-emerald-600 text-white')}>
          <Tag className="w-3.5 h-3.5" />
          {isOption ? '期权' : '股票'}
        </Badge>
      );
