LuckyTrading777 全局规则与架构活文档（v0.5.0 · 净值归真全量修正版）
...
版本说明：本版本基于 v0.4.4 全文，紧急修正了周期性盈亏（WTD/MTD/YTD）的统计算法，废弃了会导致“历史利润重复计算”的基金会计模型，回归朴素的“每日净值变动加总”模型。其余章节与 v0.4.4 保持严格一致（包含所有 55 项审计细节）。
...
1.3 非侵入式数据调整原则（Non-Invasive Adjustment） [v0.4.0 新增]
...
配置源：拆股信息存储在代码配置（如 src/lib/corporate-actions/stock-splits.ts）或独立的 Firestore 配置表中，不污染 transactions 集合。
...
2.3 日切规则（Day Cut-off Rules，日切规则） [v0.3.2 新增]
...
2.4 代码防御与治理（Code Governance） [v0.4.0 新增]
...
live（盘中）：市场处于正常交易时段（MarketOpen，市场开盘），API 获取实时价成功；
stale（待更新）：市场处于正常交易时段（MarketOpen），但 API 获取实时价失败或超时，目前使用的是上一次成功获取的价格 lastKnownPrice；
ready（开盘就绪）：市场处于开盘前特定时段（PreMarket，盘前），API 已启动并获取到最新盘前价；
close（已收盘）：市场处于收盘后时段（MarketClosed，收盘后），当前使用的是最后一次有效价格（通常是收盘价或收盘后最后一次更新）。
...
规则示意：
如果市场是 MarketClosed：M1 的状态必须是 close；
...
4.4 UI 与交互规范（UI Standards） [v0.4.0 新增]
...
实时更新中：使用 bg-green-500 animate-ping (绿色呼吸灯)。

数据陈旧 (Stale)：使用 bg-orange-500 animate-pulse (橙色呼吸灯)。
...
error -> 显示 "Degraded" (降级)。

4.4.4 格式化规范

货币 (Currency)：使用 formatCurrency，保留 2 位小数，千分位，正数带 + 号。

百分比 (Percent)：使用 formatPercent，乘以 100，保留 2 位小数，后缀 %，正数带 + 号。

单价 (Price)：动态精度，默认 2 位，持仓成本价强制 4 位小数。

空值占位：未知数据必须显示 — (em dash)，严禁显示 0。

4.4.5 排序与筛选

默认排序：按 costBasis (净现金投入) 降序排列。

三态循环：null (默认) -> asc (升序) -> desc (降序) -> null。

零持仓过滤：列表仅显示 netQty !== 0 或 todayPl > 0.001 的记录（即使清仓，若今日有盈亏也需显示）。
...
6.3.6 M6 当日盈亏归因（Attribution Analysis） [v0.4.0 重写]
...
6.9 滚动盈亏平衡价（Rolling Break-even Price） [v0.3.5 新增]
...
6.10 累计已实现盈亏（M9） [v0.4.0 新增]

定义：基于全局 FIFO 账本，累计账户历史上所有已平仓交易的 Realized PnL。

公式：Sum(All Historical Realized PnL)。

实现：基于全局 FIFO 账本计算。

6.11 周期性盈亏（M11-M13: WTD/MTD/YTD） [v0.5.0 彻底重写]

原则目标
衡量净收入增值（Net Income Increase）。即：在不考虑充提币的前提下，账户总资产在本周期内增加了多少。

核心公式（Sum of Dailies / 算术求和法）
周期盈亏定义为该周期内所有交易日 Day PnL（当日总盈亏 M6） 的算术和。

$$ \text{PeriodPnL} = \sum_{d=\text{StartDate}}^{\text{Today}} \text{M6}_{\text{Total}}(d) $$

计算逻辑：

原子单位：Day PnL (M6) 是最小统计单位，包含当天的浮盈变化和已实现收益。

历史日期：对于过去的日期，M6(d) 的值已固化（基于当时的 EOD）。

今天 (Today)：对于今天，M6(Today) 是实时的（基于 Real-time Price）。

加总：将周期内每一天的 M6 加在一起，即为该周期的总盈亏。

基准日 (Base Date Definition)
...
禁止使用 Flow + Delta 算法，防止历史低成本仓位在平仓时导致数据重复计算。

必须与日历组件的数据源保持逻辑一致（WTD ≈ Sum of Weekly Calendar Cells）。

6.12 胜率统计（M10 - Win Rate Stats） [v0.4.0 新增]

统计对象：基于 FIFO 结对的完整“回合”（Round-trip）。
...
PnLRatio = AvgWin / AvgLoss

Expectancy = (WinRate * AvgWin) - (LossRate * AvgLoss)

6.13 交易效率分析 (Transaction Analytics) [v0.4.0 新增]

交易效率：(PnL / TradingValue) * 10000 (每万美元交易产生的盈亏)。

四象限分析：资金使用量 vs 盈亏。

6.14 每日盈亏日历（M14 - Daily PnL Calendar） [v0.4.3 新增]

原则目标
以日历视图展示历史每一天的账户净值变动（Net Asset Value Change）。日历上的每一个格子，代表那一天的“净收入”。

一致性约束

Total PnL 铁律：日历中某一日期 D 的数值，必须等于该日当时的 M6 (当日盈亏)。

严禁仅显示已实现：日历不仅仅是“平仓记录表”，它必须包含当天的持仓浮动盈亏。

计算公式
$$ \text{CalendarCell}D = \text{M6}{\text{Total}}(D) $$
(注：对于历史日期，M6 = Realized + (EOD_Today - EOD_Yesterday) * Qty)
...
10. 数据流与业务边界 (Data Flow & Boundaries) [v0.4.0 新增]

10.1 缓存与刷新机制

EOD 预取：同时预取今日、昨日及周期基准日（周/月/年）的 EOD 数据。

自动回填：当前端检测到 EOD 缺失且无 Error 标记时，自动触发 API。

价格新鲜度：FRESHNESS_MS = 60_000 (60秒)。

10.2 特殊标的处理

期权 (Option)：

识别：assetType === 'option' 或 multiplier !== 1。

合约键：Symbol + Expiry + Strike + Type。

展示：金额强制乘以 multiplier（默认 100）。

交易方向扩展：

支持 BUY/BTO, SELL/STC, SHORT/STO, COVER/BTC 四种原子操作。

11. 已知偏差与技术债务 (Known Deviations & Technical Debt)

本章记录了截至 v0.5.0 版本，系统代码中尚未合规的已知问题。这些问题不代表规则变更，而是代表必须修复的技术债务。

11.1 数据字段违规（Critical）
